...
def RunGenerators(api: str, registry: str, video_registry: str, directory: str, styleFile: str, targetFilter: str, flatOutput: bool):
    ...
    if api == 'vulkansc':
        mock_icd_generated_directory = 'icd/generated-vksc'
        vulkaninfo_generated_directory = 'vulkaninfo/generated-vksc'
    else:
...
def main(argv):
    ...
    if args.generated_version:
        ...
        if args.api == 'vulkansc':
            json_files.append(common_codegen.repo_relative('icd/VkICD_mock_icd_vksc.json.in'))
        else:
        ...
        with open(common_codegen.repo_relative('CMakeLists.txt'), "r+") as f:
            data = f.read()
            f.seek(0)
            f.write(re.sub(f"project.*VERSION.*# {args.api}", f"project(Vulkan-Tools VERSION {args.generated_version}) # {args.api}", data))
            f.truncate()
