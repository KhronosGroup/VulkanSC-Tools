...
void instExtInitialize(VkInstance instance) {
...
/*
 * structure substituting VkSurfaceKHR when dumping images to disk
 */
...
#endif
#if defined(CUBE_USE_IMAGE_DUMP)
    if (strcmp(str, "image") == 0) return WSI_PLATFORM_IMAGE;
#endif
...
static void demo_draw_build_cmd(struct demo *demo, VkCommandBuffer cmd_buf) {
    ...
    VkImageMemoryBarrier image_wait_barrier = {.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER,
                                                    ...
                                                    .srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED,
                                                    .dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED,
                                                    .image = demo->surface_image.image,
                                                    .subresourceRange = {VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0, 1}};
    ...
    vkCmdCopyImageToBuffer(cmd_buf, demo->surface_image.image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, demo->staging_buffer, 1, &copy_region);
...
static void demo_prepare_display_buffer(struct demo *demo) {
    VkResult U_ASSERT_ONLY err;
    ...
    err = vkCreateBuffer(demo->device, &buffer_create_info, NULL, &demo->staging_buffer);
    assert(!err);
    demo_name_object(demo, VK_OBJECT_TYPE_BUFFER, (uint64_t)demo->staging_buffer, "DispBuffer");

    VkMemoryRequirements mem_reqs;
    vkGetBufferMemoryRequirements(demo->device, demo->staging_buffer, &mem_reqs);

    demo->staging_mem_alloc.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
    demo->staging_mem_alloc.pNext = NULL;
    demo->staging_mem_alloc.allocationSize = mem_reqs.size;
    demo->staging_mem_alloc.memoryTypeIndex = 0;

    VkFlags requirements = VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT;
    pass = memory_type_from_properties(demo, mem_reqs.memoryTypeBits, requirements, &demo->staging_mem_alloc.memoryTypeIndex);
    assert(pass);

    err = vkAllocateMemory(demo->device, &demo->staging_mem_alloc, NULL, &demo->staging_mem);
    assert(!err);
    demo_name_object(demo, VK_OBJECT_TYPE_DEVICE_MEMORY, (uint64_t)demo->staging_mem, "TexBufMemory");

    /* bind memory */
    err = vkBindBufferMemory(demo->device, demo->staging_buffer, demo->staging_mem, 0);
    assert(!err);
}
...
static void demo_draw(struct demo *demo) {
    ...
    if (demo->wsi_platform != WSI_PLATFORM_IMAGE) {
    do {
    ...
    if (demo->wsi_platform == WSI_PLATFORM_IMAGE) {
        ...
        //demo_dump_display_image(demo);
        void *data;
        err = vkMapMemory(demo->device, demo->staging_mem, 0, demo->staging_mem_alloc.allocationSize, 0, &data);
        assert(!err);

        int stb_err = stbi_write_png("cube.png", demo->width, demo->height, 4, data, demo->width * 4);
        assert(stb_err);

        vkUnmapMemory(demo->device, demo->staging_mem);
        demo_cleanup(demo);
        exit(0);
    }
...
static VkResult demo_prepare_display_image(struct demo *demo) {
    demo_prepare_display_buffer(demo);
    ...
    const VkImageCreateInfo image = {
        ...
        .samples = VK_SAMPLE_COUNT_1_BIT,
        .tiling = VK_IMAGE_TILING_OPTIMAL,
        .usage = VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT,
        .flags = 0,
    };
    ...
    demo->depth.format = display_format;

    /* create image */
    err = vkCreateImage(demo->device, &image, NULL, &demo->surface_image.image);
    assert(!err);
    demo_name_object(demo, VK_OBJECT_TYPE_IMAGE, (uint64_t)demo->surface_image.image, "SurfaceImage");

    vkGetImageMemoryRequirements(demo->device, demo->surface_image.image, &mem_reqs);
    assert(!err);

    demo->surface_image.mem_alloc.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
    demo->surface_image.mem_alloc.pNext = NULL;
    demo->surface_image.mem_alloc.allocationSize = mem_reqs.size;
    demo->surface_image.mem_alloc.memoryTypeIndex = 0;

    pass = memory_type_from_properties(demo, mem_reqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
                                       &demo->surface_image.mem_alloc.memoryTypeIndex);
    assert(pass);

    /* allocate memory */
    err = vkAllocateMemory(demo->device, &demo->surface_image.mem_alloc, NULL, &demo->surface_image.mem);
    assert(!err);
    demo_name_object(demo, VK_OBJECT_TYPE_DEVICE_MEMORY, (uint64_t)demo->surface_image.mem, "SurfaceImageMem");

    /* bind memory */
    err = vkBindImageMemory(demo->device, demo->surface_image.image, demo->surface_image.mem, 0);
    assert(!err);

    /* create image view */
    view.image = demo->surface_image.image;
    err = vkCreateImageView(demo->device, &view, NULL, &demo->surface_image.view);
    assert(!err);
    demo_name_object(demo, VK_OBJECT_TYPE_IMAGE_VIEW, (uint64_t)demo->surface_image.view, "SurfaceImageView");

    demo->swapchain_image_resources[0].uniform_memory = demo->surface_image.mem;
    demo->swapchain_image_resources[0].image = demo->surface_image.image;
    demo->swapchain_image_resources[0].view = demo->surface_image.view;

    return err;
}

static VkPipelineCacheCreateInfo demo_get_pipeline_cache_info() {
...
#include "pipeline_cache.h"
    ...
    const VkPipelineCacheCreateInfo result = {
        ...
        .initialDataSize = sizeof(cache_data),
...
#define NUM_DYNAMIC_STATES 2 /*Viewport + Scissor*/
    ...
    pipelineCache = demo_get_pipeline_cache_info();
...
static void demo_prepare_descriptor_pool(struct demo *demo) {
    ...
    const VkDescriptorPoolCreateInfo descriptor_pool = {
        ...
        .pNext = NULL,
        .maxSets = 1, // Limited to one "swapchain"image
        .poolSizeCount = 2,
...
static void demo_prepare(struct demo *demo) {
    if (demo->wsi_platform != WSI_PLATFORM_IMAGE) {
        ...
        demo_prepare_display_image(demo);
    }
    ...
    if (demo->cmd_pool == VK_NULL_HANDLE) {
        const VkCommandPoolMemoryReservationCreateInfo reservation_info = {
            ...
            .pNext = NULL,
            .commandPoolMaxCommandBuffers = 10,
            .commandPoolReservedSize = 100000000
        };
...
static void demo_init_vk(struct demo *demo) {
    ...
    if (instance_extension_count > 0) {
        ...
        for (uint32_t i = 0; i < instance_extension_count; i++) {
            ...
            if (!strcmp(VK_KHR_PORTABILITY_ENUMERATION_EXTENSION_NAME, instance_extensions[i].extensionName)) {
                // Vulkan SC doesn't support VK_KHR_PORTABILITY_ENUMERATION_EXTENSION
            }
    ...
    if (!platformSurfaceExtFound && demo->wsi_platform != WSI_PLATFORM_IMAGE) {
#if defined(VK_USE_PLATFORM_WIN32_KHR)
...
static void demo_create_device(struct demo *demo) {
    ...
    VkPipelineCacheCreateInfo pipelineCache = demo_get_pipeline_cache_info();
    ...
    VkDeviceObjectReservationCreateInfo objectReservation = {
        ...
        .semaphoreRequestCount = 10,
        .commandBufferRequestCount = 10,
        .fenceRequestCount = 10,
        .deviceMemoryRequestCount = 10,
        .bufferRequestCount = 10,
        .imageRequestCount = 10,
        .eventRequestCount = 10,
        .queryPoolRequestCount = 10,
        .bufferViewRequestCount = 10,
        .imageViewRequestCount = 10,
        .layeredImageViewRequestCount = 10,
        .pipelineCacheRequestCount = 10,
        .pipelineLayoutRequestCount = 10,
        .renderPassRequestCount = 10,
        .graphicsPipelineRequestCount = 10,
        .computePipelineRequestCount = 10,
        .descriptorSetLayoutRequestCount = 10,
        .samplerRequestCount = 10,
        .descriptorPoolRequestCount = 10,
        .descriptorSetRequestCount = 10,
        .framebufferRequestCount = 10,
        .commandPoolRequestCount = 10,
        .samplerYcbcrConversionRequestCount = 10,
        .surfaceRequestCount = 10,
        .swapchainRequestCount = 10,
        .displayModeRequestCount = 10,
        .subpassDescriptionRequestCount = 10,
        .attachmentDescriptionRequestCount = 10,
        .descriptorSetLayoutBindingRequestCount = 10,
        .descriptorSetLayoutBindingLimit = 10,
        .maxImageViewMipLevels = 4,
        .maxImageViewArrayLayers = 3,
        .maxLayeredImageViewMipLevels = 10,
        .maxOcclusionQueriesPerPool = 10,
        .maxPipelineStatisticsQueriesPerPool = 10,
        .maxTimestampQueriesPerPool = 10,
        .maxImmutableSamplersPerDescriptorSetLayout = 10
    };
...
// Create a WSI surface for the window:
    ...
    if (demo->wsi_platform == WSI_PLATFORM_WIN32) {
        ...
        err = pfnCreateWin32SurfaceKHR(demo->inst, &win32_createInfo, NULL, &demo->surface);
    }
#endif
#if defined(VK_USE_PLATFORM_WAYLAND_KHR)
    if (demo->wsi_platform == WSI_PLATFORM_WAYLAND) {
...
static void demo_init(struct demo *demo, int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        strncat(available_wsi_platforms, "image", max_str_len);
#endif
#if defined(VK_USE_PLATFORM_DISPLAY_KHR)
        if (strlen(available_wsi_platforms) > 0) {
            strncat(available_wsi_platforms, "|", max_str_len);
