...
#define LOAD_INSTANCE_FUNCTION_SC(instance, fn_ptr, fn_name) fn_ptr = (PFN_##fn_ptr)vkGetInstanceProcAddr(instance, #fn_name)

static VkResult load_vulkan_library() {
    vulkan_library = NULL;
#if defined(WIN32)
#ifdef VULKANSC
    HMODULE library = LoadLibraryA("vulkansc-1.dll");
#else
...
#elif defined(__APPLE__)
#ifdef VULKANSC
    void* library = dlopen("libvulkansc.dylib", RTLD_NOW | RTLD_LOCAL);
#else
    void* library = dlopen("libvulkan.dylib", RTLD_NOW | RTLD_LOCAL);
#endif // VULKANSC
    if (library == NULL) {
#ifdef VULKANSC
        library = dlopen("libvulkansc.1.dylib", RTLD_NOW | RTLD_LOCAL);
#else
...
#ifdef VULKANSC
        library = dlopen("/usr/local/lib/libvulkansc.dylib", RTLD_NOW | RTLD_LOCAL);
#else
        library = dlopen("/usr/local/lib/libvulkan.dylib", RTLD_NOW | RTLD_LOCAL);
#endif // VULKANSC
        if (library == NULL) {
#ifdef VULKANSC
            library = dlopen("/usr/local/lib/libvulkansc.1.dylib", RTLD_NOW | RTLD_LOCAL);
#else
...
#ifdef VULKANSC
    void* library = dlopen("libvulkansc.so.1", RTLD_NOW | RTLD_LOCAL);
#else
    void* library = dlopen("libvulkan.so.1", RTLD_NOW | RTLD_LOCAL);
#endif // VULKANSC
#ifdef VULKANSC
    if (library == NULL) library = dlopen("libvulkansc.so", RTLD_NOW | RTLD_LOCAL);
#else
...
static void load_vulkan_instance_functions(VkInstance instance) {
    ...
    LOAD_INSTANCE_FUNCTION_SC(instance, vkEnumeratePhysicalDeviceGroupsKHR, vkEnumeratePhysicalDeviceGroups);
    LOAD_INSTANCE_FUNCTION_SC(instance, vkGetPhysicalDeviceFeatures2KHR, vkGetPhysicalDeviceFeatures2);
    LOAD_INSTANCE_FUNCTION_SC(instance, vkGetPhysicalDeviceProperties2KHR, vkGetPhysicalDeviceProperties2);
    LOAD_INSTANCE_FUNCTION_SC(instance, vkGetPhysicalDeviceFormatProperties2KHR, vkGetPhysicalDeviceFormatProperties2);
    LOAD_INSTANCE_FUNCTION_SC(instance, vkGetPhysicalDeviceQueueFamilyProperties2KHR, vkGetPhysicalDeviceQueueFamilyProperties2);
    LOAD_INSTANCE_FUNCTION_SC(instance, vkGetPhysicalDeviceMemoryProperties2KHR, vkGetPhysicalDeviceMemoryProperties2);
#endif // VULKANSC
}
